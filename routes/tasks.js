...
// Get all time entries (admin only) - for time report
router.get('/admin/time-report', (req, res) => {
...
router.get('/my/active-timer', (req, res) => {
...
router.post('/:id/time/start', (req, res) => {
  ...
  if (running) {
    return res.status(400).json({ error: `You already have a timer running on "${running.task_title}". Stop it first.` });
  }

  // Use Central Time
  const now = new Date().toLocaleString('en-US', { timeZone: 'America/Chicago' });
  const ctNow = new Date(now).toISOString();

  const result = db.prepare(`
    INSERT INTO time_entries (task_id, employee_id, start_time)
    VALUES (?, ?, ?)
  `).run(req.params.id, req.session.userId, ctNow);
...
router.post('/:id/time/stop', (req, res) => {
  ...
  db.prepare('UPDATE time_entries SET end_time = ? WHERE id = ?').run(ctNow, running.id);
...
router.post('/:id/time/manual', (req, res) => {
  ...
  const result = db.prepare(`
    INSERT INTO time_entries (task_id, employee_id, start_time, end_time)
    VALUES (?, ?, ?, ?)
  `).run(req.params.id, req.session.userId, start_time, end_time);
...
// Delete time entry (own entries only, or admin)
